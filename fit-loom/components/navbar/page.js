"use client";

import React, { useRef, useEffect, useState } from "react";
import Link from "next/link";
import Image from "next/image";
import { CiShoppingCart, CiCirclePlus, CiCircleMinus } from "react-icons/ci";
import { IoIosCloseCircle } from "react-icons/io";
import { AiFillMinusCircle, AiFillPlusCircle } from "react-icons/ai";
import { IoBagCheckOutline } from "react-icons/io5";

import { useCart } from "../providers/CartProvider";

const Navbar = () => {
  const { cart, addToCart, removeFromCart, clearCart, subTotal } = useCart();
  const [isCartOpen, setIsCartOpen] = useState(false);
  const ref = useRef();

  // Listen for cart:itemAdded event to open cart when Add to Cart button is clicked
  useEffect(() => {
    const handleCartItemAdded = () => {
      openCart();
    };

    window.addEventListener("cart:itemAdded", handleCartItemAdded);

    return () => {
      window.removeEventListener("cart:itemAdded", handleCartItemAdded);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const openCart = () => {
    if (ref.current) {
      ref.current.classList.remove("translate-x-full");
      ref.current.classList.add("translate-x-0");
      setIsCartOpen(true);
    }
  };

  const closeCart = () => {
    if (ref.current) {
      ref.current.classList.remove("translate-x-0");
      ref.current.classList.add("translate-x-full");
      setIsCartOpen(false);
    }
  };

  const toggleCart = () => {
    if (isCartOpen) {
      closeCart();
    } else {
      openCart();
    }
  };
  return (
    <div className="flex flex-col md:flex-row md:justify-start justify-between items-center py-2 shadow-md sticky top-0 bg-white z-50">
      <div className="Logo mx-5">
        <Link href="/">
          <Image src="/logo.webp" alt="logo" width={200} height={40} />
        </Link>
      </div>
      <div className="Nav">
        <ul className="flex  items-center space-x-6 font-bold md:text-md ">
          <li>
            {" "}
            <Link href="/t-shirt"> T-Shirts</Link>{" "}
          </li>
          <li>
            {" "}
            <Link href="/hoodies"> Hoodies</Link>{" "}
          </li>
          <li>
            {" "}
            <Link href="/mugs"> Mugs</Link>{" "}
          </li>
          <li>
            {" "}
            <Link href="/stickers"> Stickers</Link>{" "}
          </li>
        </ul>
      </div>
      <div
        onClick={toggleCart}
        className="cursor-pointer Cart absolute right-0 mx-5 top-4"
      >
        {" "}
        <CiShoppingCart className=" text-xl md:text-3xl" />
      </div>

      {/* Side Cart */}
      <div
        ref={ref}
        className="w-full sm:w-80 md:w-96 sideCart fixed top-0 right-0 h-screen bg-white shadow-2xl transform transition-transform duration-300 translate-x-full z-50 flex flex-col"
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-pink-500 to-pink-600 px-4 sm:px-6 py-4 flex items-center justify-between border-b border-pink-400">
          <h2 className="font-bold text-lg sm:text-xl text-white flex items-center gap-2">
            <span>🛒</span>
            Shopping Cart
          </h2>
          <button
            onClick={closeCart}
            className="cursor-pointer text-white hover:text-pink-200 transition-colors text-2xl sm:text-3xl"
          >
            <IoIosCloseCircle />
          </button>
        </div>

        {/* Cart Items - Scrollable */}
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4">
          {Object.keys(cart).length == 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-12">
              <div className="text-gray-300 text-6xl mb-4">🛒</div>
              <p className="text-gray-500 font-semibold text-base sm:text-lg text-center">
                Your cart is empty!
              </p>
              <p className="text-gray-400 text-sm mt-2 text-center">
                Add some items to get started
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {Object.keys(cart).map((k) => {
                const itemTotal = (cart[k].price || 0) * cart[k].qty;
                return (
                  <div
                    key={k}
                    className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200 hover:shadow-md transition-shadow"
                  >
                    <div className="flex flex-col gap-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-800 text-sm sm:text-base mb-1 truncate">
                          {cart[k].name}
                        </h3>
                        {(cart[k].size || cart[k].variant) && (
                          <div className="flex flex-wrap gap-2 mb-2">
                            {cart[k].size && (
                              <span className="text-xs text-gray-500 bg-gray-200 px-2 py-0.5 rounded">
                                Size: {cart[k].size}
                              </span>
                            )}
                            {cart[k].variant && (
                              <span className="text-xs text-gray-500 bg-gray-200 px-2 py-0.5 rounded">
                                {cart[k].variant}
                              </span>
                            )}
                          </div>
                        )}
                        <p className="text-pink-600 font-bold text-sm sm:text-base">
                          ₹{itemTotal.toFixed(2)}
                        </p>
                        <p className="text-gray-400 text-xs">
                          ₹{cart[k].price?.toFixed(2)} each
                        </p>
                      </div>
                      <div className="flex items-center justify-between bg-white rounded-full px-3 py-1.5 border border-gray-200">
                        <AiFillMinusCircle
                          onClick={() => removeFromCart(k, 1)}
                          className="cursor-pointer text-pink-500 hover:text-pink-700 transition-colors text-lg sm:text-xl flex-shrink-0"
                        />
                        <span className="mx-3 text-gray-800 font-semibold min-w-[1.5rem] text-center text-sm sm:text-base">
                          {cart[k].qty}
                        </span>
                        <AiFillPlusCircle
                          onClick={() =>
                            addToCart(
                              k,
                              1,
                              cart[k].price || 0,
                              cart[k].name,
                              cart[k].size,
                              cart[k].variant,
                              false // Don't open cart when updating quantity in sidebar
                            )
                          }
                          className="cursor-pointer text-pink-500 hover:text-pink-700 transition-colors text-lg sm:text-xl flex-shrink-0"
                        />
                      </div>
                    </div>
                  </div>
                );
              })}
            </div>
          )}
        </div>

        {/* Footer with Subtotal and Buttons */}
        {Object.keys(cart).length > 0 && (
          <div className="border-t border-gray-200 bg-gray-50 px-4 sm:px-6 py-4 space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-gray-700 font-semibold text-sm sm:text-base">
                Subtotal:
              </span>
              <span className="text-pink-600 font-bold text-lg sm:text-xl">
                ₹{subTotal?.toFixed(2) || "0.00"}
              </span>
            </div>
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                onClick={clearCart}
                className="flex items-center justify-center w-full sm:flex-1 text-gray-700 bg-gray-200 border-0 py-2 px-3 focus:outline-none hover:bg-gray-300 rounded-lg text-xs sm:text-sm font-semibold transition-colors duration-200"
              >
                Clear Cart
              </button>
              <Link
                href="/checkout"
                onClick={closeCart}
                className="flex items-center justify-center w-full sm:flex-1 text-white bg-pink-500 border-0 py-2 px-3 focus:outline-none hover:bg-pink-600 rounded-lg text-xs sm:text-sm font-semibold transition-colors duration-200 shadow-md hover:shadow-lg"
              >
                <IoBagCheckOutline className="mr-1.5 text-sm" />
                Checkout
              </Link>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default Navbar;
